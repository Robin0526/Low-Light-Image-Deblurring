# train.py
# 增强版训练脚本：添加验证功能、学习率调度、混合损失，并保存示例图片
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # 添加clamp确保输出在0-1范围内
        return torch.clamp(self.final_conv(x), 0, 1)
...
class HybridLoss(nn.Module):
    """
    混合损失函数，结合L1损失、感知损失和SSIM损失
    参考文献:
    - "Image Quality Assessment: From Error Visibility to Structural Similarity" (SSIM)
    - "Perceptual Losses for Real-Time Style Transfer and Super-Resolution" (感知损失)
    """
    def __init__(self, alpha=0.5, beta=0.3, gamma=0.2, device='cuda'):
        super().__init__()
    ...
    def forward(self, pred, target):
        ...
        if self.beta > 0:
            total_loss += self.beta * percep
        if self.gamma > 0:
            total_loss += self.gamma * ssim_loss_val
        
        return total_loss, {'l1': l1.item(), 'percep': percep if isinstance(percep, float) else percep.item(), 'ssim': ssim_loss_val.item()}
...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # 保存模型检查点
        checkpoint_path = os.path.join(args.ckpt_dir, f"unet_epoch{epoch}.pth")
        ...
        if val_loss < best_val_loss:
            ...
            best_checkpoint_path = os.path.join(args.ckpt_dir, "unet_best.pth")
    ...
    # 保存最终模型
    final_checkpoint_path = os.path.join(args.ckpt_dir, "unet_final.pth")
...
def save_visual_comparison(input_img, output_img, target_img, epoch, psnr_val, ssim_val, save_dir):
    ...
    axes[0].set_title("Low Blur Input")
    axes[0].axis('off')
    ...
    axes[2].set_title("High Sharp Target")
    axes[2].axis('off')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="低光去模糊 UNet 模型训练")
